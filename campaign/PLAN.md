...
| M9 | upstream | **The pattern upstream.** PLAYBOOK §4 rewritten for the fork model; the general parts of `INSTANCE-PLAYBOOK.md` moved upstream | **landed 2026-09-24.** The pattern went into the family playbook — `PLAYBOOK.md` and `INSTANCES.md`, which live in `~/Sortilege/VTT/` beside the VTT repos (owner, 2026-09-24; first committed to `sortilege-vtt-teeth` `44a2c20`, then moved out of it): `PLAYBOOK.md` §4 rewritten (*a campaign is an instance: a fork of its system's VTT*) and `INSTANCES.md`, the general guide — this file's content, folded with what M1–M8 and the player-page work proved (config keys carried by hand after a pull; both origins in `ALLOWED_ORIGIN`; redeploy the Worker after an op change; a live session needs two browsers; the hook lives in `sortilege-vtt-l5r5e` and is ported before another VTT is forked). `campaign/INSTANCE-PLAYBOOK.md` is now a pointer and what is Portents' own |
...
| 58 | **A technique opens to its full text; the roller runs in the table's order** (owner, 2026-09-24; upstream I13, I14) — what it is for, TN, the dice it comes to, ring, skill, Void (only while there is one), assistance, *Roll* across the bottom. **A hidden TN gives its Void point automatically, and so is a choice made for each check:** *Roll* waits for a TN (*Hidden* or 1–8) unless a technique or initiative set one | The owner asked for no button on a concealed TN. With "not set" also meaning "hidden", every check rolled without picking a TN would have granted a Void point; making the TN a per-check choice keeps the automatic grant and removes the accident. Proven with Norikage: *Breaking Blow* opens *Roll* at TN 1 ready to roll; the Void row gone at 0 Void points and back at 3. |
| 59 | **The player's roller starts at TN 2** (owner, 2026-09-24; upstream I15), and returns to it after each resolve. Replaces decision 58's "Roll waits for a TN": the accident it guarded against (a hidden TN by default, granting Void) cannot happen when the default is 2; *Hidden* is still a deliberate tap | The player's page only; the GM's roller keeps no default (decision 27). |
| 60 | **The player's page: a Conflict tab while the GM has one on, Gear with notes, an Advancement page, no Sheet tab** (owner, 2026-09-24; upstream I16). A conflict is the GM's to start and end, for the whole party, from the Party panel. Advancement saves as a new current version (op `advancePartyMember`) — so **the Worker was redeployed** (`portents-vtt` version `2d2eae28`; the bundle carries the op; a room from the site's origin 200, a foreign origin 403). Next steps for the advancement screen are upstream's `TODO.md` | Norikage in Portents: tabs *Play · Roll · Gear*; the Advancement page on his record (11 earned, 9 spent, 2 available; Air 1→2 6 XP, Fire 2→3 9 XP); the GM's *Duel* → his *Conflict* tab with the duel's initiative (Meditation, TN 1), *Ready Bō*, the unarmed strikes; *End the conflict* cleared it. Layer build OK, check_npcs 18/18, check_norikage 0 differ. |
| 61 | **The Roll tab: skills as a grouped list with dots, and Roll is the bar's own tab name, red on that tab** (owner, 2026-09-24; upstream I17) | No op changed; the Worker is not redeployed. |
| 62 | **Session Seven (23 Sep) entered from its transcript.** Chronicle: *A Pilgrim in One&rsquo;s Own Home*, nine scenes, the table&rsquo;s own words, Juri&rsquo;s working shown only as Norikage saw it. Behind the Veil rolled forward to *as of Session Seven* — his purification, the charge on to the Three Sisters, the fight, the yokai story, Juri&rsquo;s turned blow — and the played prep replaced by **Session Eight — White Flower at nightfall**; the seed follows it (one scene, the played ambush dropped). Juri&rsquo;s card: Campaign Status moved on; `check_npcs.py` now reports a moved-on Campaign Status as play, not failure, every statblock field still strict | The owner settled the two the transcript did not: **2 XP** for the session (13 earned, 9 spent, 4 unspent), and **Norikage** said *&ldquo;It silenced the year I was born&rdquo;* — so Juri now knows the silence is his age. |
| 63 | **The site's menu on a phone** (owner: "the main page menu isn't mobile friendly"; upstream I18): one row — the name and a menu button naming the tab; the campaign's seven tabs marked `group: 'campaign'`, so the open menu draws a line between them and the books | Header 459 → 58px at 375px. |
| 64 | **Owner's corrections to Session Seven**: Juri carries a **bō**, not a bow; what she did was the **Essence Eternal** inversion (<code>celestial-realms-mechanics.ttrpg</code> p. 119; the owner's "Eternal Essence") — a GM note only. Behind the Veil names it and quotes its Activation, Effect and Magnitude verbatim; the Chronicle shows only what Norikage saw, and loses the *Theology (Void)* roll box for her, the one mechanical tell | The Chronicle is the player's; what the working was is the veil's. |
| 65 | **Session Seven's two arc scenes marked played** in the live GM page's arc (owner, 2026-09-24). That browser's arc predates the Session Eight seed, so it doesn't have *White Flower at nightfall* (the seed never overwrites an arc the campaign already has) | The seed fills only keys a campaign has never had (`engine/state.js` seed). |
| 66 | **The Lore pane from any browser: Tailscale Funnel, with a token checked by `lore serve`** (owner chose option A of four, 2026-09-24: Funnel; a Worker in front of a Cloudflare Tunnel via Workers VPC; a Cloudflare Tunnel on a Cloudflare-hosted hostname; the homeproxy VPS). Address `https://helheim.tail926c54.ts.net`; the token in `~/.config/l5r-lore/serve.env`, typed into Settings by the GM. l5r-lore: the Bearer check, `--require-token`, a 15 s socket timeout and the `l5r-lore` user service. Upstream: the pane's token field (its decision 35), merged here. The runbook is `campaign/LORE-PROXY.md` | $0, and nothing new to install: Tailscale was already running and logged in here. The index never leaves the desktop. The trade-off is that the desktop's tailnet name becomes public and the token check is in the Python server, not in front of it. Hence the timeout, and `--require-token`, which keeps the service from ever starting open. |
