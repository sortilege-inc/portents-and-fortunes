...
tabs, its state document in the GM's Notes pane. No build step for the pages; the scripts in
`source/` regenerate what is generated.

| Path | What it is |
|------|------------|
| `docs/` | The campaign's documents, drawn by the site tabs: `home`, `characters` and `norikage` (the *Norikage* tab), `chronicle`, `map`, `atlas` (map-pin targets are its `id`s), `lore`, `personae` (the cast's masthead); and `state` — **Behind the Veil**, the GM's campaign-state document, in the Notes pane behind its spoiler gate |
...
| `pack/seed.json` | The arc and the ambush from the Session Seven prep, seeded once into the GM's campaign (generated by `source/build_seed.py`) |
