// system/l5r5e/gm-text.js — the GM's own material, kept in the pack and edited in the GM tabs
// (the family standard: an instance's "behind the veil" document lives in the tabs, not in a file).
//
...
//
// Where it lives (system/l5r5e/ops.js): `gm` — { overview, places, people, pc, rules: [section],
...
window.L5RGmText = (function () {
  ...
  function aboutRow(ids) {
    ...
    return el('div', { class: 'gm-about muted small' }, ['About: '].concat([].concat(...ids.map((id, i) => {
      const r = D && (D.entity(id) || D.records().find((x) => x.id === id));
      const m = !r && (S().party || []).find((p) => p.name === id);
      const b = r ? el('button', { class: 'ref', type: 'button', onclick: () => window.L5ROpenEntity(r.id) }, [r.name])
  ...
  const WHERE = { overview: 'overview', places: 'places', people: 'cast', pc: 'party', rules: 'rules' };
  function search(q) {
