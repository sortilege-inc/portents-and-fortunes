...
| The GM tabs (Behind the Veil, moved into the pack: the campaign's overview, Scenes, Threads, Places, notes on people and characters, rulings) | **Reference, not essay** — governed by the `gm-prep-notes` skill, which outranks this file for that surface. Everything is tagged (`SET` / `AGREED` / `SOURCE` / `YOURS` / `OPEN` / `MINE` / `NOTE`) and structure beats prose where structure is clearer. No second person, no first person, no commentary about the document. |
