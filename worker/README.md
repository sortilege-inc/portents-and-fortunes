...
`engine/ops.js` (and `system/l5r5e/ops.js`) the browser uses, validates them by role (the GM
...
```bash
cd worker && npm install && npx wrangler dev --port 8792   # local; the app on localhost talks to it
